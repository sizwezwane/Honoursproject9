...
def groundPlans(meshFileName):
    ...
    drawBot = ImageDraw.Draw(im)
    drawBot.line((300, 900, 1350, 900), fill=128, width=2)
    drawBot = ImageDraw.Draw(im)
    drawBot.line((300, 900, 312.5, 875), fill=128, width=2)
    drawBot = ImageDraw.Draw(im)
    drawBot.line((325, 900, 312.5, 875), fill=128, width=2)
    drawBot.text((310, 900), "E1", fill=(0, 0, 0), font=font)

    drawtop = ImageDraw.Draw(im)
    drawtop.line((300, 150, 1350, 150), fill=128, width=2)
    drawBot = ImageDraw.Draw(im)
    drawBot.line((300, 150, 312.5, 175), fill=128, width=2)
    drawBot = ImageDraw.Draw(im)
    drawBot.line((325, 150, 312.5, 175), fill=128, width=2)
    drawBot.text((300, 120), "E3", fill=(0, 0, 0), font=font)

    drawleft = ImageDraw.Draw(im)
    drawleft.line((125, 200, 125, 825), fill=128, width=2)
    drawleft = ImageDraw.Draw(im)
    drawleft.line((125, 200, 150, 212.5), fill=128, width=2)
    drawleft = ImageDraw.Draw(im)
    drawleft.line((125, 225, 150, 212.5), fill=128, width=2)
    drawBot.text((85, 200), "E4", fill=(0, 0, 0), font=font)

    drawright = ImageDraw.Draw(im)
    drawright.line((1400, 200, 1400, 825), fill=128, width=2)
    drawleft.line((1400, 200, 1375, 212.5), fill=128, width=2)
    drawleft = ImageDraw.Draw(im)
    drawleft.line((1400, 225, 1375, 212.5), fill=128, width=2)
    drawBot.text((1410, 200), "E2", fill=(0, 0, 0), font=font)

    # s1
    drawBot = ImageDraw.Draw(im)
    relbot = 275
    drawBot.line((300 - 50, 900 - relbot, 1350, 900 - relbot), fill=128, width=2)
    drawBot = ImageDraw.Draw(im)
    drawBot.line((300 - 50, 900 - relbot, 312.5 - 50, 875 - relbot), fill=128, width=2)
    drawBot = ImageDraw.Draw(im)
    drawBot.line((325 - 50, 900 - relbot, 312.5 - 50, 875 - relbot), fill=128, width=2)
    drawBot.text((300 - 50, 900 - relbot), "S1", fill=(0, 0, 0), font=font)

    drawright = ImageDraw.Draw(im)
    relrig = 600
    drawright.line((1400 - relrig, 200, 1400 - relrig, 825), fill=128, width=2)
    drawleft.line((1400 - relrig, 200, 1375 - relrig, 212.5), fill=128, width=2)
    drawleft = ImageDraw.Draw(im)
    drawleft.line((1400 - relrig, 225, 1375 - relrig, 212.5), fill=128, width=2)
